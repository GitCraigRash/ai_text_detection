...
X_train = df.loc[df["mask"]==True]
y_train = df.loc[df["mask"]==False]
X_train = np.array(X_train)
y_train = np.array(y_train)

mask = np.random.choice([True, False], size=len(test_df), p=[0.8, 0.2])
X_test = test_df.loc[df["mask"]==True]
y_test = test_df.loc[df["mask"]==False]
X_test = np.array(X_test)
y_test = np.array(y_test)

#df_padded = None

X_train[:5],len(y_train),X_test[:5],len(y_test)
